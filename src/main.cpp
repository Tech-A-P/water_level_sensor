...
void setup()
{
   ...
   delay(2000); // wait that rele is closed and sim 900 take power and than turn on the module
...
//CALCOLO DISTANZA
  ...
  cm_notC = 20 - average*346/20000;            //calcola la distanza NON corretta con la velocità del suono: velocità standard di 320m/s
  cm = 20 - average*soundSpeed/20000;          //calcola la distanza in cm
/*
...
*/       
  ...
  delay(100);
    ...
    // TCP Client GET, send a GET request to the server andsave the reply invia la misura corretta & non corretta & la temperatura
        ...
        const char URL[72];    //add the measure to the url of the client GET
        ...
        //String url = "/update?api_key=2CUGSICX631J8CLR&field1="+ String(cm, 2)+ "&field2=" + String (cm_notC , 2) + "&field3=" + String (Temperature , 2); //convert measure (double) in string , indicate the decimal number. 
        delay(1000);
        url.toCharArray(URL,72);
        //Serial.println("url");  
        Serial.print(URL);
        ...
        pinMode(9, OUTPUT);      //turn OFF the module form program è necesario saldare il jumper R13 sulla scheda e fa tutto lui. 
   digitalWrite(9,LOW);
   delay(1000);
   digitalWrite(9,HIGH);
   delay(2000);
...
void loop()
{
  ...
  serialhwread();
  // Read for new byte on NewSoftSerial.f
  serialswread();
 //Serial.println("loop");
 //delay(500);
