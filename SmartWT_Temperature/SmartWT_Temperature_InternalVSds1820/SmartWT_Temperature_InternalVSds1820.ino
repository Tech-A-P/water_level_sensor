...
// define pin
#define TEMPERATURE_PIN 5                        // temperature pin

// Set bus of communication
OneWire oneWire(TEMPERATURE_PIN);       
DallasTemperature sensors(&oneWire);    

//Variabili per lettura interna senore di temperatura
...
const String THINGSPEAK_API_KEY = "WYTEST9NUGKJ4MER";     // write API key from thingspeak
const char* THINGSPEAK_ENDPOINT_ROOT = "api.thingspeak.com";
const String THINGSPEAK_FIELD_NUMBER = "1";      // ThingSpeak field index for the water level value
const String THINGSPEAK_BATTERY_NUMBER = "2";    // ThingSpeak field index for the battery value 
const String THINGSPEAK_IntTemp_NUMBER = "5";    // ThingSpeak field index for the IntTemp value
const String THINGSPEAK_DS1820Temp_NUMBER = "6";    // ThingSpeak field index for the DS1820Temp value
const String THINGSPEAK_rawADC_NUMBER = "7";    // ThingSpeak field index for the rawADC value for calibration
...
//for DS1920
sensors.begin();
sensors.setResolution(12);          // max precision
sensors.setWaitForConversion(true); // wiat until connection is closed
...
ARDUtemp = readInternalTemp_NanoEvery(NUM_SAMPLES, rawavg);
Serial.print(F("Temperatura interna Arduino: "));
Serial.println(ARDUtemp, 1);
Serial.print(F("rawavg: "));
Serial.println(rawavg);

// exeternal temperatyre DS1820
sensors.requestTemperatures(); 
Temperature = sensors.getTempCByIndex(0); 
Serial.print("TEMPERATURE: ");  // returns it to the serial monitor
...
void sendData() {
  ...
  String url = "/update?api_key=" + THINGSPEAK_API_KEY + 
                ...
                "&field" + THINGSPEAK_IntTemp_NUMBER + "=" + String(ARDUtemp , 1) +
                "&field" + THINGSPEAK_DS1820Temp_NUMBER + "=" + String(Temperature , 1) +
                "&field" + THINGSPEAK_rawADC_NUMBER + "=" + String(rawavg , 1); 
...
// funzione che resituisce un valore int che è la temperatura misurata all'interno di arduino.
// non è la stessa del nano 328P perchè qui cambiano i registri con l'every
float readInternalTemp_NanoEvery(uint16_t samples, float &rawavg) {
  Serial.print("samples");
  Serial.println(samples);
  Serial.print("ADC0.RES");
  Serial.println(ADC0.RES);

  uint32_t sum = 0;

  ADC0.CTRLA = ADC_ENABLE_bm;
  ADC0.CTRLC = ADC_REFSEL_INTREF_gc | ADC_PRESC_DIV64_gc;
  ...
  rawavg = (float)sum / samples;

  //questa è la formula calibrata sulla base dei test fatti a febbraio 2026. adesso una bella cosa sarebbe calibrarla con i dati di un anno. 
  float temperatureC = 0.2623*rawavg-242.86;
